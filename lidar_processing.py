...
def simToRobot(x_in_simu, z_in_simu):
    ...
    #print(config.state[0], config.state[1])
...
def ICPSVD(fixedX, fixedY, movingX, movingY):
    ...
    """ fixedt = [[x, y, 0.0] for x, y in zip(fixedX, fixedY)]
    movingt = [[x, y, 0.0] for x, y in zip(movingX, movingY)] """
    ...
    """ Finalement, l'utilisation de np.array n'est peut etre pas plus rapide si on doit refaire cette conversion derriere 
        A voir en détail comment est fait la conversion """
    ...
    for i in range(10):
        ...
        # TODO: faire mieux pour le threshold sur l'erreur
        if err < 200:
...
def getLidarData():
    ...
    for idx, i in enumerate(point_cloud):
        ...
        # Placement des données dans le repère de la simulation 
...
def updateCorrectedLidarData(reqR, reqT):    
    """ Returns all lidar points after correcting its position relative to the map"""
    # Tableau de stockage des données
...
def getRotationTranslationICP(reqR, reqT):
    ...
    return reqT[0], reqT[1], angle_R*180/pi